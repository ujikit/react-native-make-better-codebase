...
//local import here
import styles from './Semua.styles';
import SemuaLogic from './Semua.logic';
      ...
      <View style={styles.wrapSearch}>
        ...
        <View>
          <Text style={[styles.descText]}>
            {I18n.t('total')} : {data.waterState.allUnitStatus.data.length}{' '}
            {I18n.t('unit')}
          </Text>
        </View>
      </View>
      <FlatList
        data={
          data.searchValue.length
            ? data.waterState.allUnitStatus.data.filter((item) =>
                item.unitId.includes(data.searchValue)
              )
            : data.waterState.allUnitStatus.data
        }
        extraData={data}
        ...
        contentContainerStyle={
          data.waterState.allUnitStatus.data.length === 0 && styles.wrapEmtpy
        }
        ListEmptyComponent={
          <ScreenMessage
